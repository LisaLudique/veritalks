<?php 
    ...
    // set variables
    $limit = 20;
    $numresults = CS50::query("SELECT * FROM posts WHERE published='Y' AND prospective_students='1'");
    ...
    $result = CS50::query("SELECT id, post, answer, time, upvotes FROM posts WHERE published='Y' AND prospective_students='1' order by time desc limit $offset, $limit");
    
    // apologize if results don't exist
    if (!$result)
    {
    ...
    foreach ($result as $row) 
    {
	    ...
	    echo $row['time'];
	    echo "<center>";
	    echo "<a href='prospective_students_upvote.php?id={$id}&upvote={$vote}' onclick=\"return confirm('Like this?')\">". $row['upvotes'] . " people found this helpful. Do you?</a>";
	    echo "</center>";
	    echo "</p>";
    } 
    ...
    print "<a href=prospective_students_user.php?offset=$prevoffset>PREV</a> &nbsp; \n";
    ...
    for ($i = 1;$i <= $pages; $i++)
    {
        ...
        print "<a href=prospective_students_user.php?offset=$newoffset>$i</a> &nbsp; \n";
    }
    ...
    print "<a href=prospective_students_user.php?offset=$newoffset>NEXT</a><p>\n";
