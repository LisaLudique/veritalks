<?php 
    ...
    // set variables
    $limit = 20;
    $numresults = CS50::query("SELECT * FROM posts WHERE published='Y'");
    ...
    $result = CS50::query("SELECT id, post, answer, time, upvotes FROM posts WHERE published='Y' order by time desc limit $offset, $limit");
    ...
    foreach ($result as $row) 
    {
        ...
        echo $row['time'];
        echo "<center>";
        echo "<a href='upvote.php?id={$id}&upvote={$vote}' onclick=\"return confirm('Like this?')\">". $row['upvotes'] . " people found this helpful. Do you?</a>";
        echo "</center>";
        echo "</p>";
    } 
    ...
    print "<a href=user.php?offset=$prevoffset>PREV</a> &nbsp; \n";
    ...
    for ($i=1;$i<=$pages;$i++)
    {
        ...
        print "<a href=user.php?offset=$newoffset>$i</a> &nbsp; \n";
    }
    ...
    print "<a href=user.php?offset=$newoffset>NEXT</a><p>\n";
